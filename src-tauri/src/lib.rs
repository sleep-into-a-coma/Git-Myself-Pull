...
use chrono::Utc;
use models::{AppState, Repository, Settings};
...
async fn update_one(app: AppHandle, id: String) -> Result<(), String> {
    ...
    let (repo, proxy_mode, proxy_address) = {
        ...
        repo.is_running = true; repo.last_attempt = Some(Utc::now()); repo.last_status = "正在更新…".into();
        (repo.clone(), proxy_mode, proxy_address)
    };
    publish(&app); store.log(format!("[{}] 开始检查", repo.name));
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![get_state, save_repository, delete_repository, detect_branch, choose_folder, open_folder, save_settings, clear_logs, update_repository, update_all, check_app_update, install_app_update, exit_app])
